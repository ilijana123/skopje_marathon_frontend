...
export const ContestantsTab: React.FC<ContestantsTabProps> = () => {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      setFilteredContestants(contestantsArray); // Remove the .filter if you want ALL contestants
      // OR if you only want paid contestants:
      // setFilteredContestants(contestantsArray.filter((p: Contestant) => p.isPaid));
      
      console.log('Fetched contestants:', contestantsArray); // Add for debugging
    } catch (error) {
      console.error("Fetch error:", error);
      setFilteredContestants([]); // Set empty array on error
    } finally {
      setLoading(false);
    }
  };
